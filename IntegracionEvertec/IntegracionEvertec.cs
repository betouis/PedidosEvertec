...
namespace PediosEvertec.IntegracionEvertec
{
    ...
    {
        private string login = System.Configuration.ConfigurationManager.AppSettings["login"];
        private string tranKey = System.Configuration.ConfigurationManager.AppSettings["tranKey"];
        private string urlString = System.Configuration.ConfigurationManager.AppSettings["urlString"];

        public async Task<ResponseEvertecPago> InitAsync(string urlRedireccion, Pedido pedido, string producto, double valor)
        {
            return await AutenticacionAsync(urlRedireccion, pedido, producto, valor);
        }
        public async Task<ResponseEvertecPago> AutenticacionAsync(string urlRedireccion, Pedido pedido, string producto, double valor)
        {
            ...
            try
            {
                ...
                EnvioDatos dataEnviar = new EnvioDatos()
                {
                    ...
                    returnUrl = urlRedireccion + "pedidoes/Edit/" + pedido.id.ToString(),
        ...
        public async Task<ResponseEvertecPago> EstadoAsync(Pedido pedido)
        {
            string nonce = (new Random()).GetHashCode().ToString();
            string nonceBase64 = Base64(nonce);
            string seed = (DateTime.Now.AddMinutes(4)).ToString("yyyy-MM-ddTHH:mm:sszzz");
            ...
            try
            {
                ...
                HttpResponseMessage response = await cliente.PostAsync(url + pedido.requestId, content);
                if (response.IsSuccessStatusCode)
                {
                    string resultadoToken = await response.Content.ReadAsStringAsync();
                    ResponseEvertecPago res = ser.Deserialize<ResponseEvertecPago>(resultadoToken);
